...
Screens — RNTL:
- **AC 006-7** — `TerazScreen` renders the **active** place's reading: with active =
  location (fake reading city 'Kraków', index 4) shows Kraków/4; after `setActive`
  to a Warszawa station (fake reading city 'Warszawa', index 42) shows Warszawa/42.
  No `MOCK_PLACE` in the tree.
...
- **AC 006-10** — the **Teraz tab-bar tint** derives from the active reading's index
  (`scene(activeReading.index).key`), not `MOCK_PLACE`: a navigator rendered with a
  fake active reading of index N tints the Teraz tab `scene(N).key`; `MOCK_PLACE` is
  deleted from the app render path.
...
- **AC 006-5, 006-6** — `src/shared/place/__tests__/*` (RNTL, fake store + sourceForPlace).
